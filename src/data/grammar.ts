...
const PP_MC: MCItem[] = [
  ...
  // ── tricky cases ──
  {
    id: "pp-mc-16", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "The on-call engineer acknowledged the alert twenty minutes ___.",
    options: ["since", "ago", "already", "yet"],
    correct: 1,
    explanation: "'Ago' counts back from now to a specific moment — it always pairs with Past Simple ('acknowledged … ago'). 'Since' introduces a starting point, not an endpoint.",
  },
  {
    id: "pp-mc-17", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "The server crashed at 9 AM today, so we rolled back the release immediately.",
    options: ["has crashed", "crashed", "crashes", "had crashed"],
    correct: 1,
    explanation: "'At 9 AM' pins the crash to an exact clock time — even though 'today' is an unfinished day, a specific time within it forces Past Simple.",
  },
  {
    id: "pp-mc-18", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "Breaking news: our team ___ just shipped the feature the client has been waiting for.",
    options: ["just shipped", "has just shipped", "just ships", "had just shipped"],
    correct: 1,
    explanation: "Announcements use Present Perfect because the focus is on the result that is relevant right now, not on the exact moment the action happened.",
  },
  {
    id: "pp-mc-19", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "We can't deploy — the CI pipeline ___ and we need to fix it first.",
    options: ["failed", "has failed", "fails", "had failed"],
    correct: 1,
    explanation: "The failure has a present consequence ('we can't deploy now') → Present Perfect. If you were narrating a past sequence ('The pipeline failed, so we stopped the release'), Past Simple would be used.",
  },
  {
    id: "pp-mc-20", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "In 2021, the platform ___ four major releases and doubled its user base.",
    options: ["has shipped", "shipped", "ships", "had shipped"],
    correct: 1,
    explanation: "'In 2021' is a closed calendar year — it is completely in the past → Past Simple, regardless of how many events occurred.",
  },
  {
    id: "pp-mc-21", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "___ you ever managed a distributed team across more than three time zones?",
    options: ["Did", "Have", "Do", "Had"],
    correct: 1,
    explanation: "This is a life-experience question with no specific time mentioned → Present Perfect ('Have you ever managed'). If asking about a specific role, we'd say 'Did you manage … at [company]?'",
  },
  {
    id: "pp-mc-22", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "Maria ___ as tech lead for five years — she left the role last quarter.",
    options: ["has worked", "worked", "works", "had worked"],
    correct: 1,
    explanation: "'She left the role last quarter' confirms the period is finished → Past Simple. Use Present Perfect only when the state is still ongoing ('has worked … and still does').",
  },
  {
    id: "pp-mc-23", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "Performance ___ significantly since we migrated to the new database cluster.",
    options: ["improved", "has improved", "improves", "had improved"],
    correct: 1,
    explanation: "'Since we migrated' anchors the improvement to a past event and shows it continues to the present → Present Perfect.",
  },
  {
    id: "pp-mc-24", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "Last Tuesday, I logged into the legacy system and ___ three hours reading the docs.",
    options: ["have spent", "spent", "spend", "had spent"],
    correct: 1,
    explanation: "Past narrative: 'Last Tuesday, I logged … and spent' — once a story is set in a specific past time, all subsequent verbs use Past Simple throughout the sequence.",
  },
  {
    id: "pp-mc-25", topic: "present-perfect", level: 2, type: "multiple-choice",
    sentence: "It's only 11 AM and the team ___ already merged five pull requests today.",
    options: ["merged", "has", "had", "was"],
    correct: 1,
    explanation: "'Today' is an unfinished period and 'already' signals Present Perfect. The sentence expands to 'has already merged' — the auxiliary 'has' is what the blank requires.",
  },
];
...
const PP_FB: FBItem[] = [
  ...
  // ── tricky cases ──
  {
    id: "pp-fb-16", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "The on-call engineer ___ the alert twenty minutes ago.",
    answer: "acknowledged",
    hint: "use: acknowledge (Past Simple — 'ago' always forces Past Simple)",
    explanation: "'Ago' counts back from the present to a fixed point in the past — it always requires Past Simple, never Present Perfect.",
  },
  {
    id: "pp-fb-17", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "The reviewer ___ approved the PR yet — it's still waiting.",
    answer: "hasn't",
    hint: "use: hasn't (Present Perfect negative auxiliary — 'yet' signals unfinished state)",
    explanation: "'Yet' in a negative statement shows the action is expected but not done — the state is current and relevant → Present Perfect negative ('hasn't approved').",
  },
  {
    id: "pp-fb-18", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "Great news — we ___ just secured the Series A funding!",
    answer: "have",
    hint: "use: have (Present Perfect — announcement, result matters now)",
    explanation: "Announcements of results that matter right now use Present Perfect. 'Just' reinforces that the news is fresh and currently relevant.",
  },
  {
    id: "pp-fb-19", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "In 2022, the engineering team ___ four major platform updates.",
    answer: "released",
    hint: "use: release (Past Simple — 'in 2022' is a closed year)",
    explanation: "'In 2022' is a finished calendar year entirely in the past → Past Simple, even when multiple events happened within it.",
  },
  {
    id: "pp-fb-20", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "Our CTO ___ the engineering department for the past eight years.",
    answer: "has led",
    hint: "use: lead (Present Perfect — still leading today)",
    explanation: "'For the past eight years' describes a period that starts in the past and continues to now — the CTO is still in the role → Present Perfect.",
  },
  {
    id: "pp-fb-21", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "___ you ever managed a cross-functional team without a formal PM?",
    answer: "Have",
    hint: "use: Have (Present Perfect — life experience, no specific time)",
    explanation: "Life-experience questions with 'ever' and no specific time reference use Present Perfect ('Have you ever managed'). Adding a time ('in your last job') would force Past Simple ('Did you manage').",
  },
  {
    id: "pp-fb-22", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "Load times ___ by 40% since we enabled server-side caching.",
    answer: "have dropped",
    hint: "use: drop (Present Perfect — improvement started at a past event, continues now)",
    explanation: "'Since we enabled' marks the start of an ongoing change. The improvement runs from that past event to now → Present Perfect.",
  },
  {
    id: "pp-fb-23", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "Our app ___ 100,000 downloads today — the marketing campaign is working!",
    answer: "has reached",
    hint: "use: reach (Present Perfect — milestone announced today, day still ongoing)",
    explanation: "'Today' is an unfinished time period and the milestone is newsworthy right now → Present Perfect. If the day were over or a specific time were given, Past Simple would apply.",
  },
  {
    id: "pp-fb-24", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "We can't push to main — someone ___ accidentally force-pushed and broken the branch.",
    answer: "has",
    hint: "use: has (Present Perfect auxiliary — current consequence prevents action now)",
    explanation: "The force-push has a present consequence ('we can't push now') → Present Perfect ('has … broken'). The exact time of the push is irrelevant; the current blocker is what matters.",
  },
  {
    id: "pp-fb-25", topic: "present-perfect", level: 3, type: "fill-blank",
    sentence: "Did you send the API keys to the client? — Yes, I ___ already sent them this morning.",
    answer: "have",
    hint: "use: have (Present Perfect — 'already' + unfinished morning, result is now complete)",
    explanation: "'Already' + 'this morning' (still the same morning) → Present Perfect. The focus is on the current state: the keys are sent and the client has them.",
  },
];
...
const PP_TR: TRItem[] = [
  ...
  // ── tricky cases ──
  {
    id: "pp-tr-16", topic: "present-perfect", level: 4, type: "transformation",
    original: "The board approved the new product roadmap.",
    instruction: "Rewrite as a company announcement where the result — not the timing — is what matters.",
    answer: "The board has approved the new product roadmap.",
    explanation: "Present Perfect is used for announcements when the result ('the roadmap is now approved and in effect') matters more than when exactly the approval happened.",
  },
  {
    id: "pp-tr-17", topic: "present-perfect", level: 4, type: "transformation",
    original: "The senior engineer has left the company.",
    instruction: "Rewrite to specify it happened three months ago.",
    answer: "The senior engineer left the company three months ago.",
    explanation: "'Ago' requires Past Simple — it specifies a fixed point in the past counting back from now. Present Perfect is incompatible with 'ago'.",
  },
  {
    id: "pp-tr-18", topic: "present-perfect", level: 4, type: "transformation",
    original: "We switched to microservices in 2021. Deployment time has improved a lot.",
    instruction: "Combine into one sentence using 'since'.",
    answer: "Deployment time has improved a lot since we switched to microservices in 2021.",
    explanation: "'Since' + a past event clause → Present Perfect in the main clause. The improvement started at the switch and continues now.",
  },
  {
    id: "pp-tr-19", topic: "present-perfect", level: 4, type: "transformation",
    original: "Have you ever used Terraform to manage infrastructure?",
    instruction: "Rewrite to ask specifically about their previous job.",
    answer: "Did you use Terraform to manage infrastructure in your previous job?",
    explanation: "Adding a specific context ('previous job') moves the question to a finished time period → Past Simple. Life-experience questions with no time reference use Present Perfect.",
  },
  {
    id: "pp-tr-20", topic: "present-perfect", level: 4, type: "transformation",
    original: "This week, the team has shipped the new onboarding flow.",
    instruction: "Rewrite to say it happened last week.",
    answer: "Last week, the team shipped the new onboarding flow.",
    explanation: "'Last week' is a closed, finished period → Past Simple. 'This week' is still ongoing, which is why Present Perfect was valid in the original.",
  },
  {
    id: "pp-tr-21", topic: "present-perfect", level: 4, type: "transformation",
    original: "We have just fixed the production bug.",
    instruction: "Rewrite as a past narrative: the fix happened at 2 AM and it was a tough session.",
    answer: "We fixed the production bug at 2 AM — it was a tough debugging session.",
    explanation: "Adding 'at 2 AM' (specific time) turns the sentence into a past narrative. All verbs in the sequence ('fixed', 'was') use Past Simple.",
  },
  {
    id: "pp-tr-22", topic: "present-perfect", level: 4, type: "transformation",
    original: "Maria worked as tech lead for five years.",
    instruction: "Rewrite to show she is still tech lead today.",
    answer: "Maria has worked as tech lead for five years.",
    explanation: "When the 'for + period' is still ongoing, switch to Present Perfect. Past Simple ('worked') implies the role ended; Present Perfect ('has worked') shows it continues.",
  },
  {
    id: "pp-tr-23", topic: "present-perfect", level: 4, type: "transformation",
    original: "Someone deleted the main branch.",
    instruction: "Rewrite to show the current consequence: the team cannot merge anything right now.",
    answer: "Someone has deleted the main branch — the team cannot merge anything right now.",
    explanation: "Present Perfect highlights that the past action has a present consequence. The addition of 'the team cannot merge anything' makes the current impact explicit.",
  },
  {
    id: "pp-tr-24", topic: "present-perfect", level: 4, type: "transformation",
    original: "Did you already send the API keys to the client?",
    instruction: "Rewrite using Present Perfect to focus on whether the task is done now.",
    answer: "Have you already sent the API keys to the client?",
    explanation: "'Already' in questions about current status uses Present Perfect in formal and British English. The focus shifts from 'when did you do it?' to 'is it done now?'",
  },
  {
    id: "pp-tr-25", topic: "present-perfect", level: 4, type: "transformation",
    original: "That was the hardest incident I had ever dealt with.",
    instruction: "Rewrite in the present tense using the standard superlative + Present Perfect pattern.",
    answer: "That is the hardest incident I have ever dealt with.",
    explanation: "The standard pattern for superlative experience is 'is/are the [superlative] I have ever [V3]'. 'Had ever dealt' is Past Perfect, which is only needed inside a past narrative.",
  },
];
